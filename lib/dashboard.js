...
// --- Persistent exclusions/preferences ---
...
export async function saveExclusion({ userId, reason, grantName, category }) {
  if (!supabase || !userId || !reason) return;
  const existing = await getExclusions(userId);
  // Avoid duplicates (same reason text)
  if (existing.some((e) => e.reason === reason)) return;
  const updated = [
    ...existing,
    { reason, grant_name: grantName || null, category: category || null, added_at: new Date().toISOString() },
  ];
  const { error } = await supabase
    .from("bg_profiles")
    .update({ preferences: updated, updated_at: new Date().toISOString() })
    .eq("id", userId);
  if (error) console.error("Failed to save exclusion:", error);
  return updated;
}

export async function removeExclusion({ userId, reason }) {
  if (!supabase || !userId) return;
  const existing = await getExclusions(userId);
  const updated = existing.filter((e) => e.reason !== reason);
  const { error } = await supabase
    .from("bg_profiles")
    .update({ preferences: updated, updated_at: new Date().toISOString() })
    .eq("id", userId);
  if (error) console.error("Failed to remove exclusion:", error);
  return updated;
}
